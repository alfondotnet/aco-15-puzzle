...
class ACOProblem(object):
    ...
    def pheromone_evaporation(self):
        ''' pheromone_evaporation
            ...
            Evaporates equally all edges of the graph
        '''
        
        for edge in self.graph.edges(data=True):       
            edge[2]['weight'] = edge[2]['weight'] * (1-self.p)
    ...
    def update_graph(self, list_solutions):
        ...
        for solution in list_solutions:
            ...
            for edge in g.edges(data=True):
                edge[2]['weight'] = self.pheromone_update_criteria(solution) # Positive feedback
            

            self.graph = nx.compose(g, self.graph)
    ...
    def run(self):
        ...
        while not(self.end_condition()):
            ...
            print("Updating graph (new nodes and pheromone)")
            
            print (solutions)
            raw_input()
            self.update_graph(solutions)
